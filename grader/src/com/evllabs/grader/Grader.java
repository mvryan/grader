		...
		if (args.length == 0) {
			...
			for (Entry<String, List<Histogram>> entry : rankEntries) {
				...
				System.out.println(entry.getKey() + " "
						+ authors.get(entry.getKey()));
				for (int i = 0; i < current.size(); i++) {
					StringBuilder resultBuilder = new StringBuilder();
					resultBuilder.append(i + 1);
					Set<Entry<String, Integer>> rankSet = current.get(i)
							.entrySet();
					for (Entry<String, Integer> rankEntry : rankSet) {
						resultBuilder.append(" ").append(rankEntry.getKey())
								.append(":").append(rankEntry.getValue())
								.append("(")
								.append(100 * (rankEntry.getValue() / total))
								.append("%)");
					}
			...
			for (String author : authorList) {
				...
				for (Histogram current : ranks.get(author)) {
					...
					for (String element : authorList) {
						lineBuilder.append(current.get(element)).append(",");
					}
